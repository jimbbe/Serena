...
- [ ] `ConversationStore` port interface with `findOrCreateConversation`, `getConversation`, `appendMessage`, `listMessages`
- [ ] `InMemoryConversationStore` implements the port and passes unit tests
- [ ] `ProcessChannelInboundMessage` auto-creates conversation and appends inbound messages for resolved identities
- [ ] `ChannelInboundResult` includes `conversation` field with `id`, `status`, `messageCount`
- [ ] Scenario runner propagates `conversationId` between steps (continuity test passes)
- [ ] Outbound drafts are recorded as outbound messages
- [ ] All existing tests pass (no regressions)
- [ ] New tests cover store operations, use case integration, scenario continuity
- [ ] `docs/simulation-api.md` documents `conversation` in response schemas
